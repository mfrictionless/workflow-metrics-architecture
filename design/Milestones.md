...
- **Test:** `make up` launches the `ods-postgres` service from `docker-compose.yml`; `ods/ddl/` is mounted into Postgres's `/docker-entrypoint-initdb.d/` directory (the official image's auto-init mechanism — any `.sql`/`.sh` there runs once, in alphabetical order, only when the data directory is empty); Postgres executes the mounted `001_schema.sql` on first startup, creating `files`, `persons`, `users`, `file_actions`, `parties`, `audit_events` with all foreign keys and CHECK constraints intact (enumerated columns — `status`, `action_code`, `action_type`, `role` — using ALL CAPS values), and every column carrying a `COMMENT`. `persons` holds PII (`display_name`/`email`/`ssn_last4`); `users` holds operational identity (`team_name`/`is_external_vendor_flag`) via a FK to `persons`; `parties.person_id` and `file_actions.sent_user_id`/`received_user_id` are real, FK-enforced references, not unvalidated integers — see [D013](Decisions.md#d013).
- **Acceptance:** From a clean checkout, `make up` brings the ODS up with no manual `psql -f` step, and: all 6 tables exist (`information_schema.tables`); FK constraints from `file_actions`/`parties`/`audit_events` to `files` are present; FK constraints from `users` to `persons`, from `parties` to `persons`, and from `file_actions`/`audit_events` to `users` are present; CHECK constraints on the 4 enumerated columns exist and only accept ALL CAPS values (a lowercase insert, e.g. `role = 'borrower'`, is rejected); every column across all 6 tables has a non-empty `COMMENT`. Restarting the container without `make down` (volume persists) does not re-run or error on the init script, per Postgres's own init-once behavior.
...
- **Automated Test Plan:**
  - *Integration:* `tests/integration/test_schema_init.sh` — brings the stack up via `scripts/compose_up.sh`, waits for readiness, then: (a) confirms `/docker-entrypoint-initdb.d/001_schema.sql` exists in the container (the mount is wired correctly); (b) queries `information_schema.tables` for all 6 tables; (c) queries `information_schema.table_constraints`/`pg_constraint` for the expected FKs, including `users`→`persons`, `parties`→`persons`, and `file_actions`/`audit_events`→`users`; (d) attempts a lowercase enum insert on each of the 4 enumerated columns and asserts each is rejected (regression-testing the ALL CAPS fix made during the original schema build, now automated instead of manual); (e) queries `pg_description` and asserts every column of all 6 tables has a non-empty comment, not just a couple of spot-checked ones. Tears down after.
- **Manual Test Plan:**
  ...
  - `psql`/`\d+ <table>` in, confirm all 6 tables, their constraints, and column comments are present.
...
- **Test:** `make seed` — a separate, explicit command — applies `ods/seed/seed.sql` against the running ODS (piped into `psql` via `docker compose exec`), inserting one closed file's truncated 4-step workflow (Apply → Process → Sign → Record and close), with `persons`/`users` rows for every role involved, `parties` rows assigning each role, and RACI-correct sender/receiver per steps 1, 3, 9, 12 in [Home-Refinance-Workflow.md](Home-Refinance-Workflow.md) — see [D013](Decisions.md#d013). Kept out of `ods/ddl/` (not mounted into `docker-entrypoint-initdb.d`) so `make up` alone brings up an **empty** ODS — seed data is never automatically mixed into whatever the simulator (M1.3+) generates.
- **Acceptance:** From a clean checkout, `make up` brings up an ODS with 0 files. Running `make seed` afterward results in exactly 1 file (`status='CLOSED'`) with its 4 `file_actions` rows, each `sent_at < received_at`, sender/receiver resolved through `users`/`persons` to the correct party role; `files.closed_at` equals the `RECORDING` step's `received_at`, and that step's `received_user_id` is the Autoclose System service principal's `user_id` (A5).
...
- **Out-of-scope:**
  ...
  - **Idempotency of repeated `make seed` calls** — it's an explicit, additive command; running it twice inserts a second file (confirmed in manual testing), though the shared `persons`/`users` rows themselves are inserted idempotently (`WHERE NOT EXISTS`) and are not duplicated. Not otherwise guarded against; revisit if that becomes a real workflow problem.
- **Automated Test Plan:**
  - *Integration:* `tests/integration/test_seed_data.sh` — brings the stack up via `scripts/compose_up.sh`, asserts **0 files** present (proving `make up` alone stays empty); runs `scripts/seed.sh`; then verifies exactly 1 file, 4 `file_actions` rows with expected `action_code`s in order, every row's `sent_at < received_at`, correct sender/receiver roles per step (resolved via `users`/`persons`), `RECORDING.received_user_id` equal to the Autoclose System principal's `user_id`, and `files.closed_at` matching `RECORDING.received_at` exactly (the seed script captures one `now()` value via `\gset` and reuses it for both, avoiding two separate `now()` calls drifting apart). Tears down after.
- **Manual Test Plan:**
...
- **Test:** `make simulate` (default count from `.env`'s `SIMULATOR_FILE_COUNT=5`; override per-run with `make simulate COUNT=1000`) runs the simulator in a one-off container (`docker compose run`) on the compose network, inserting `COUNT` new, independent, fully-closed files — same truncated 4-step workflow and RACI-correct sender/receiver as M1.2's seed data. `BORROWER` gets a fresh `persons`/`users` row per file (it's the file's customer); the other 5 roles (`LOAN_OFFICER`, `LOAN_PROCESSOR`, `TITLE_AGENT`, `NOTARY`, `COUNTY_RECORDER`) draw from a small reusable pool (`POOL_SIZE_PER_ROLE` in `.env`), since those are internal/vendor staff who plausibly handle many files — see [D013](Decisions.md#d013). Row-generation logic (`simulator/workflow.py`) is pure and dependency-free, separated from the DB-writing wrapper (`simulator/simulate.py`, the only module importing `psycopg2`) so it can be unit-tested without a database.
- **Acceptance:** `make simulate COUNT=n` produces exactly `n` new closed files, each with its own 4 `file_actions` and 6 `parties` rows, correct roles per step, no orphaned rows, no cross-file mixups, and `RECORDING.received_user_id` resolving to the Autoclose System principal's `user_id` (A5). Additive across runs, like `make seed`. `make simulate` alone (no `COUNT=`) uses the `.env` default.
...
- **Out-of-scope:**
  ...
  - The full 12-step workflow — M7.
  - Configurable/variable pool composition beyond a single `POOL_SIZE_PER_ROLE` count (e.g. per-role pool sizes, weighted selection) — one flat pool size for all 5 pooled roles is sufficient for this working example.
- **Automated Test Plan:**
  - *Fast:* `tests/check_simulator_logic.sh` — runs `simulator/tests/test_workflow.py` via stdlib `unittest` (no pip install, no Docker): action sequence and order, `sent_at < received_at`, sender/receiver roles per step match Home-Refinance-Workflow.md, `closed_at` matches the terminal step's `received_at`, terminal step's receiver is the SYSTEM role's `user_id` (not a party), 6 parties (each `party.person_id` wired from the `role_ids` `build_file` is given, not fabricated internally), and no `file_number` collisions across two calls to `build_file`.
  - *Integration:* `tests/integration/test_simulator.sh` — brings the stack up, confirms 0 files before simulating, runs `COUNT=3 ./scripts/simulate.sh`, asserts 3 closed files each with exactly 4 `file_actions` / 6 `parties`, no orphans, correct roles per step (joined within the correct file, proving no cross-file mixups), `RECORDING.received_user_id` resolving through `users`/`persons` to the Autoclose System principal's email (not a `parties` row, since SYSTEM isn't a party role); runs once more with `COUNT=1` and confirms the count is additive (4 total).
- **Manual Test Plan:**
...
- **Acceptance:** `make up` brings the broker up and it accepts client/admin connections; creating 6 topics (`files`, `persons`, `users`, `file_actions`, `parties`, `audit_events`) and listing them succeeds; no Zookeeper process/container exists anywhere in the stack. Restarting the container without `make down` preserves the cluster ID and created topics (confirmed in manual testing — no errors, no re-formatting).
...
- **Automated Test Plan:**
  - *Integration:* `tests/integration/test_kafka.sh` — brings the broker up, waits for it to accept connections (`kafka-broker-api-versions.sh`), creates the 6 topics, asserts all 6 appear in the topic list, and confirms no Zookeeper container is running anywhere in the stack.
- **Manual Test Plan:**
  - `make up`, `docker compose exec kafka /opt/kafka/bin/kafka-topics.sh --bootstrap-server localhost:9092 --list` — confirm the 6 topics.
...
- **Automated Test Plan:**
  - *Integration:* `tests/integration/test_jdbc_sink.sh` — brings the stack up, registers both connectors, waits for the sink connector/task to reach `RUNNING`, runs `make seed`, polls for the row to land (the pipeline is asynchronous), then asserts each of the 6 seeded Raw tables' row count matches its ODS counterpart, `_cdc_op`/`_cdc_ts`/`_sink_ts` are non-null on every `raw_files` row, and `_cdc_op='c'`.
- **Manual Test Plan:**
...
- **Manual Test Plan:**
  - `make down && make up && make register-connector`, `make simulate COUNT=1`, then `psql` into `warehouse-postgres`: compare `_cdc_source_txn_id` across `raw_files`, `raw_persons`, `raw_users`, `raw_parties`, `raw_file_actions` for that file's rows — confirm they match.
...
**M2.8 — Status-lifecycle updates: simulator emits updates, CDC lands them as Raw versions**
- **Test:** The simulator (M1.3) is extended from a single all-final insert to a realistic two-phase lifecycle, so the CDC pipeline finally exercises `UPDATE` events — every test through M2.6 was insert-only (`_cdc_op='c'`), leaving Debezium's/​the sink's `op=u` path unverified. Per file, `simulator/simulate.py`'s `insert_file()` now runs **two transactions**: (1) insert the file as `WIP` (`closed_at` NULL), its 6 `parties`, and its 4 `file_actions` with `sent_at` set but `received_at` NULL — one commit, preserving the single-transaction cross-table correlation M2.6 relies on for the insert rows; (2) `UPDATE` the file to `CLOSED` (+`closed_at`) and `UPDATE` each `file_action` to set `received_at` (and `received_user_id` per RACI, the Autoclose System principal's `user_id` for the terminal RECORDING step, per A5/[D013](Decisions.md#d013)) — a second commit. `parties` stay insert-only (no natural parties mutation). Because ODS `UPDATE`s modify rows **in place**, the ODS final state per file is unchanged (1 file, 4 `file_actions`, 6 `parties`); the extra versions appear only in the append-only Raw landing via CDC. Debezium emits `op=u` with a full `after` image (default `REPLICA IDENTITY` suffices — only the `after` state is landed, not a full `before`; `REPLICA IDENTITY FULL` stays a M2.7 concern), and the JDBC sink (`insert.mode=insert`) lands each as a new Raw row. `make seed` stays insert-only (a fixed fixture — keeps M1.2/M2.5's seed-based tests stable); the lifecycle is a `make simulate` behavior.
- **Acceptance:** After `make up`, `make register-connector`, `make simulate COUNT=1`: the **ODS** holds exactly 1 file (`status='CLOSED'`, `closed_at` set), 4 `file_actions` (each `received_at` set, `sent_at < received_at`, RECORDING `received_user_id` resolving to the Autoclose System principal), 6 `parties` — the in-place-updated final state, identical to what the old insert-only simulator produced. In **Raw**: `raw_files` has 2 rows for that `file_id` (`_cdc_op` `c` then `u`, the update's `_cdc_ts` ≥ the create's — the two events can share a millisecond, so `>=` not strictly `>`; the `c` row `status='WIP'`, the `u` row `status='CLOSED'`); each `raw_file_actions` key has 2 rows (`c` with `received_at` NULL, `u` with `received_at` set); `raw_parties` has 6 rows, all `_cdc_op='c'`. The insert-phase rows across all three tables share one `_cdc_source_txn_id`; the update-phase rows share a second, different one.
- **Dependencies:** M1.3 (simulator), M2.5 (sink + Raw landing), M2.6 (`_cdc_op`/`_cdc_source_txn_id` + transaction metadata). Independent of the deferred M2.7 (before-image), which stays deferred.
- **Out-of-scope:**
  - Durable open/in-progress files — every file still ends `CLOSED` within one `make simulate` run; this adds the `WIP`→`CLOSED` transition, not long-lived open files (overlaps M7).
  - Making `make seed` emit updates — kept insert-only deliberately, so M1.2/M2.5 seed-based tests stay valid.
  - `REPLICA IDENTITY FULL` / before-images — the sink lands only the `after` state; full before-image capture stays M2.7's (deferred) concern.
  - Multiple intermediate updates per step, or party updates — one update per file and per `file_action`; `parties` insert-only.
- **Automated Test Plan:**
  - *Fast:* `simulator/tests/test_workflow.py` extended — `workflow.py` gains a pure `open_state(file_data)` that derives the WIP snapshot from `build_file`'s closed output (status `WIP`, `closed_at`/`received_at`/`received_user_id` NULL, sent side and ordering unchanged, input not mutated); unit-assert both that snapshot and the unchanged closed state (`sent_at < received_at`, terminal RECORDING `received_user_id` NULL, `closed_at` = RECORDING `received_at`).
  - *Integration:* `tests/integration/test_lifecycle_updates.sh` — stack up, both connectors registered, `make simulate COUNT=1`, wait for the expected Raw row counts, then assert the per-key Raw version counts and `_cdc_op` sequence above, the ODS final state, and the two-transaction correlation (insert rows share one `_cdc_source_txn_id`, update rows another).
  - *Revised:* `tests/integration/test_transaction_metadata.sh` (M2.6) — updated for the lifecycle: filters to `_cdc_op='c'` rows when asserting the cross-table single-transaction correlation (the insert phase), and accounts for the new per-key Raw row counts rather than expecting exactly one row per key. Other M2.x integration tests use `make seed` (insert-only), so they are unaffected.
- **Manual Test Plan:**
  - `make up`, `make register-connector`, `make simulate COUNT=1`.
  - `psql` into the ODS — confirm 1 `CLOSED` file, 4 completed `file_actions`, 6 `parties` (final state).
  - `psql` into `warehouse-postgres`: `select _cdc_op, status, _cdc_ts from raw_files where file_id = <id> order by _cdc_ts` — confirm two rows, `c`/`WIP` then `u`/`CLOSED`.
  - Compare `_cdc_source_txn_id` across the `c` rows of all three tables (one shared value) vs. the `u` rows (a second shared value).

### M3: Metric compute (dbt)

**M3.1 — dbt project scaffold**
- **Test:** A dbt Core project (`warehouse/dbt/`) connects to `warehouse-postgres` and declares the 6 existing Raw tables (`raw_files`, `raw_file_actions`, `raw_persons`, `raw_users`, `raw_parties`, `raw_audit_events` — M2.5/M2.6) as dbt sources, with no models yet. Run via a new `dbt` service in `docker-compose.yml` — `ghcr.io/dbt-labs/dbt-postgres:1.8.latest` (the official dbt Labs image with the Postgres adapter pre-installed), `profiles: ["tools"]` like the `simulator` service, invoked via `docker compose run`, not a long-running container. This image has no `arm64` build (confirmed by inspecting its manifest — every tag checked is `amd64`-only), so the service requires `platform: linux/amd64` (Rosetta emulation on Apple Silicon) — the same constraint Debezium's own docs flagged for `quay.io/debezium/connect` builds, now hit directly rather than just noted.
...
- **Dependencies:** M2.4 (`warehouse-postgres` exists); M2.5/M2.6 (the 6 Raw tables + their `_cdc_*` columns exist, for the source declaration to point at).
- **Out-of-scope:**
...
- **Manual Test Plan:**
  ...
  - Inspect `warehouse/dbt/models/staging/__sources.yml` — confirm all 6 Raw tables are declared with their actual column names matching `warehouse/ddl/`.
...
- **Superseded acceptance criterion (M3.2):** This milestone's Acceptance above describes `make dbt-run` completing with "0 of 0 models" — true when M3.1 was built, no longer true once M3.2 adds the first real models. `tests/integration/test_dbt_scaffold.sh`'s corresponding assertion was updated in M3.2 to check for a clean `"Completed successfully"` instead of the zero-model-specific `"Nothing to do"` string. This paragraph is left as an accurate historical record of what M3.1 verified at the time; M3.2 records the test change itself.

**M3.2 — Staging models**
- **Test:** Six dbt staging models — `stg_files`, `stg_file_actions`, `stg_parties`, `stg_persons`, `stg_users`, `stg_audit_events` — each built from the corresponding `raw` source (M3.1), collapsing the append-only Raw landing to exactly one current row per business-entity key (`file_id`, `file_action_id`, `party_id`, `person_id`, `user_id`, `audit_event_id`). Dedup is `row_number() over (partition by <key> order by _cdc_ts desc) = 1` — the highest-`_cdc_ts` row wins, which collapses the multiple Raw versions a key now accrues once the simulator emits updates (M2.8): a file's `WIP` insert and `CLOSED` update land as two `raw_files` rows and staging keeps the latest (`CLOSED`); it likewise drops byte-for-byte redeliveries (at-least-once delivery lands the same event twice). Genuine out-of-order arrival does not occur here — Debezium keys each message by primary key and per-key Kafka ordering is preserved — so dedup defends against redelivery + multiple versions, not reordering. The six models share identical dedup logic, so it is factored into one macro (`macros/dedup_latest.sql`, parameterized by relation + partition key, defaulting `order by _cdc_ts`) rather than repeating the window function three times. Each model then selects only its business columns (the ODS-mirroring columns from `warehouse/ddl/001_raw_schema.sql`), dropping the Raw metadata (`_cdc_op`/`_cdc_ts`/`_sink_ts`/`_cdc_txn_id`/`_cdc_source_txn_id`) — those are landing concerns, not business data. Materialized as **views**, configured per-folder in `dbt_project.yml` (`models: workflow_metrics: staging: +materialized: view`) — the materialization strategy resolved in [D015](Decisions.md#d015), and the per-folder config that M3.1 deferred to here. Each model carries a properties `.yml` (`stg_files.yml`, etc.) declaring `unique` + `not_null` tests on its key — the project's first `dbt test`s (M3.1 had zero models to test).
- **Acceptance:** `dbt run` builds all six staging views; `dbt test` passes (`unique` + `not_null` on each key). Dedup is proven primarily against **real multi-version data from M2.8's lifecycle simulator**, with one synthetic case for the redelivery scenario the pipeline doesn't produce deterministically:
  - *Multi-version collapse (real):* after `make register-connector` + `make simulate COUNT=1`, `raw_files` holds 2 rows for the file (`c`/`WIP`, `u`/`CLOSED`) and each `raw_file_actions` key 2 rows — yet `stg_files` has exactly 1 row per `file_id` (`status='CLOSED'`, the latest `_cdc_ts`) and `stg_file_actions` exactly 1 per `file_action_id` (with `received_at` set).
  - *Redelivery collapse (synthetic):* inserting a byte-for-byte duplicate `raw_files` row (same `file_id`, same `_cdc_ts`) for a reserved `file_id` (e.g. `999999`, which the simulator never generates) still yields exactly 1 `stg_files` row for it — covering at-least-once redelivery, which otherwise only arises non-deterministically on connector restart/rebalance.
  - *No entity lost or duplicated:* `select count(distinct file_id) from raw_files` equals `select count(*) from stg_files` (and the same shape for `file_action_id`/`party_id`).
- **Dependencies:** M3.1 (dbt project, the `raw` source at `models/staging/__sources.yml`, and the `dbt`/`dbt-run` tooling exist); M2.8 (the lifecycle simulator produces genuine multiple-versions-per-key in Raw, so dedup is tested against real data, not only synthetic rows).
- **Out-of-scope:**
  - Any intermediate (`int_*`) or marts (`fct_*`/`dim_*`) models — M3.3 onward.
  - Carrying `_cdc_txn_id`/`_cdc_source_txn_id` (M2.6) or any `_cdc_*` metadata into staging — dropped here; no business use at this layer. Revisit only if a real need emerges (e.g. the deferred M2.7 before-image work).
  - True incremental materialization — views only, per D015; revisit only if `dbt run` duration or warehouse load becomes a real constraint while Raw volume is trivial.
  - `DELETE` handling — the ODS never hard-deletes (Technical-Design §2), so staging assumes `_cdc_op` is only `c`/`u`; if deletes are ever introduced, staging would need to filter or tombstone them. (`UPDATE` is now produced and tested for real via M2.8.)
  - A dedicated `staging` schema — staging views land in `public` alongside Raw, per D014/D015; schema separation stays deferred.
- **Automated Test Plan:**
  - *Integration:* `tests/integration/test_stg_dedup.sh` — stack up, both connectors registered, `make simulate COUNT=1` (M2.8 lifecycle → 2 Raw versions per file / `file_action` key). Additionally inserts one byte-for-byte duplicate `raw_files` row for a reserved `file_id` (`999999`) via `psql` to cover the redelivery case. Runs `docker compose run --rm dbt run`, then asserts against `stg_files`/`stg_file_actions`: exactly 1 row per key (checked for all 4 real `file_action_id` keys, not just the file); the surviving `raw_files` row is the `CLOSED` (latest `_cdc_ts`) version and the surviving `raw_file_actions` rows all have `received_at` set; the reserved-`file_id` duplicate collapses to 1; and `count(distinct file_id) in raw_files == count(*) in stg_files` (same for `file_action_id`/`party_id`). Finally runs `docker compose run --rm dbt test` and asserts it passes. Tears down after.
  - *Revised:* `tests/integration/test_dbt_scaffold.sh` (M3.1) — its "dbt run with zero models" assertion (`grep -q "Nothing to do"`) is no longer true once staging models exist; changed to assert `dbt run` reports `"Completed successfully"` instead, verified against real `dbt run` output rather than assumed — still proves a clean, non-erroring run without pinning to a model count that keeps growing through M3.3/M3.4.
- **Manual Test Plan:**
  - `make up`, `make register-connector` (confirm both connectors `RUNNING`), `make simulate COUNT=1`.
  - `docker compose run --rm dbt run` — confirm `stg_files`, `stg_file_actions`, `stg_parties` build.
  - `psql` into `warehouse-postgres`: `select _cdc_op, status from raw_files where file_id = <id> order by _cdc_ts` shows 2 rows (`c`/`WIP`, `u`/`CLOSED`), yet `select count(*) from stg_files where file_id = <id>` returns 1 (the `CLOSED` row). Confirm `count(distinct file_id) from raw_files` equals `count(*) from stg_files`.
  - `docker compose run --rm dbt test` — confirm the `unique`/`not_null` tests pass.
...
- **Test:** A lightweight auth mock maps a test credential to a `person_id` (or set of `party_id`s a person holds via `parties.person_id`)
