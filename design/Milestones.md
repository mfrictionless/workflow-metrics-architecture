...
- **Automated Test Plan:**
  - *Integration:* `tests/integration/test_stg_dedup.sh` — stack up, both connectors registered, `make simulate COUNT=1` (M2.8 lifecycle → 2 Raw versions per file / `file_action` key). Additionally inserts one byte-for-byte duplicate `raw_files` row for a reserved `file_id` via `psql` to cover the redelivery case. Runs `docker compose run --rm dbt run`, then asserts against `stg_files`/`stg_file_actions`: exactly 1 row per key; the surviving `raw_files` row is the `CLOSED` (latest `_cdc_ts`) version and the surviving `raw_file_actions` row has `received_at` set; the reserved-`file_id` duplicate collapses to 1; and `count(distinct file_id) in raw_files == count(*) in stg_files` (same for the other keys). Finally runs `docker compose run --rm dbt test` and asserts it passes. Tears down after.
- **Manual Test Plan:**
