...
**M3.2 — Staging models**
- **Test:** Three dbt staging models — `stg_files`, `stg_file_actions`, `stg_parties` — each built from the corresponding `raw` source (M3.1), collapsing the append-only Raw landing to exactly one current row per business-entity key (`file_id`, `file_action_id`, `party_id`). Dedup is `row_number() over (partition by <key> order by _cdc_ts desc) = 1` — the highest-`_cdc_ts` row wins, which collapses the multiple Raw versions a key now accrues once the simulator emits updates (M2.8): a file's `WIP` insert and `CLOSED` update land as two `raw_files` rows and staging keeps the latest (`CLOSED`); it likewise drops byte-for-byte redeliveries (at-least-once delivery lands the same event twice). Genuine out-of-order arrival does not occur here — Debezium keys each message by primary key and per-key Kafka ordering is preserved — so dedup defends against redelivery + multiple versions, not reordering. The three models share identical dedup logic, so it is factored into one macro (`macros/dedup_latest.sql`, parameterized by relation + partition key, defaulting `order by _cdc_ts`) rather than repeating the window function three times. Each model then selects only its business columns (the ODS-mirroring columns from `warehouse/ddl/001_raw_schema.sql`), dropping the Raw metadata (`_cdc_op`/`_cdc_ts`/`_sink_ts`/`_cdc_txn_id`/`_cdc_source_txn_id`) — those are landing concerns, not business data. Materialized as **views**, configured per-folder in `dbt_project.yml` (`models: workflow_metrics: staging: +materialized: view`) — the materialization strategy resolved in [D015](Decisions.md#d015), and the per-folder config that M3.1 deferred to here. Each model carries a properties `.yml` (`stg_files.yml`, etc.) declaring `unique` + `not_null` tests on its key — the project's first `dbt test`s (M3.1 had zero models to test).
- **Acceptance:** `dbt run` builds all three staging views; `dbt test` passes (`unique` + `not_null` on each key). Dedup is proven primarily against **real multi-version data from M2.8's lifecycle simulator**, with one synthetic case for the redelivery scenario the pipeline doesn't produce deterministically:
...
- **Out-of-scope:**
  - `stg_audit_events` — no downstream model needs it yet (M3.3's intermediate layer joins only `files`/`file_actions`/`parties`); add it when one does, per lazy creation.
