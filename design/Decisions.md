...
## D011 — JDBC sink: Debezium's own connector, hand-written Raw DDL, RegexRouter for table naming (2026-07-14)

**Change:** `change/m2.5-jdbc-sink-connector`

**Motivation.** M2.5 needs Kafka topic messages actually landing in the warehouse as Raw tables, closing the CDC pipeline (source → Kafka → sink) built across M2.1–M2.4.

**Design delta:** `design/Milestones.md` M2.5 rewritten from the draft to reflect what building it actually required (schema-evolution plan replaced by hand-written DDL — see below). `design/Technical-Design.md` §9 — `warehouse/` flipped to `exists`, a milestone earlier than planned (M3 no longer needs to create it).

**Artifacts:** `warehouse/ddl/001_raw_schema.sql` — the 4 Raw tables (business columns mirroring `ods/ddl/001_schema.sql`, plus `_cdc_op`/`_cdc_ts`/`_sink_ts`; no constraints). `docker-compose.yml` — mounts `./warehouse/ddl:/docker-entrypoint-initdb.d:ro` on `warehouse-postgres`. `cdc/debezium-jdbc-sink.json` — the sink connector config. `scripts/register_connector.sh` — generalized from registering one hardcoded file to looping over every `cdc/*.json`. `Makefile` `register-connector` — comment updated to describe both connectors. `tests/integration/test_jdbc_sink.sh`.

**Reversal:** if Silver (M3.2) needs different Raw metadata than `_cdc_op`/`_cdc_ts`/`_sink_ts` provide, the DDL and SMT chain are both small and easy to extend — no connector-level redesign needed.

**Validation:** choices and rejected alternatives —
- **Debezium's own JDBC sink (`io.debezium.connector.jdbc.JdbcSinkConnector`), not Confluent's `kafka-connect-jdbc`.** Already bundled in the `quay.io/debezium/connect` image (confirmed by inspecting the image's plugin directory during M2.3) — no new image or plugin install. Apache 2.0 licensed, sidestepping the Confluent Community License question entirely.
- **Hand-written Raw DDL, reversing the draft's original plan to rely on the connector's schema evolution.** Empirically, `_sink_ts` cannot be produced by schema evolution at all — it's not a value on the Kafka message, so there's nothing for the connector to infer a column from. Bolting it on afterward via `ALTER TABLE ... ADD COLUMN _sink_ts DEFAULT now()` is racy: the table is created lazily on the *first* message for that topic, at an unpredictable time, so a script can't know when it's safe to run the `ALTER`, and any rows landed between table-creation and the `ALTER` would get silently backfilled to the `ALTER`'s timestamp rather than their true insert time — wrong data, not a loud failure. Discussed with the owner mid-build (a real design fork, not a unilateral call): landed on hand-written DDL because it matches how production CDC-to-warehouse pipelines actually do this — tools like Fivetran/Airbyte treat metadata columns (`_fivetran_synced`, `_airbyte_extracted_at`) as part of a fixed table contract, not inferred from message content, precisely to avoid this timing dependency. `schema.evolution=none` in the connector config makes this explicit: the DDL is authoritative, the connector only inserts.
- **`_cdc_op`/`_cdc_ts` via an SMT chain (`ExtractNewRecordState` with `add.fields=op,source.ts_ms`, then `ReplaceField$Value` to rename the resulting `__op`/`__source_ts_ms` to `_cdc_op`/`_cdc_ts`), confirmed empirically field-by-field.** Neither the connector's config surface nor its README documented these field names; found by registering a throwaway connector against a real topic and inspecting the resulting table's actual columns — the `add.fields` naming convention (`__<field>`) isn't part of the connector's own advertised config.
- **`collection.table.format` does not work in this connector build (3.6.0.Final) despite being a real, accepted config key** — set it to `raw_${table}`, registered, produced data, and the resulting table was still named after the sanitized topic (`ods_public_files`), not `raw_ods_public_files`. Confirmed by testing with and without the setting on an otherwise-identical connector; the setting was accepted (visible in `GET .../config`) but silently had no effect on the created table name. Worked around with a `RegexRouter` SMT (`transforms.retopic`) that renames the topic itself (`ods.public.<table>` → `raw_<table>`) before the sink ever sees it — a standard, well-documented Kafka Connect mechanism, unlike the connector-specific naming config that didn't work.
- **A second, unrelated timing bug found first: the sink's `topics.regex` subscription doesn't notice newly created topics for up to `metadata.max.age.ms` (Kafka consumer default: 5 minutes).** Registering the sink connector before the source connector had produced any messages meant `ods.public.*` topics didn't exist yet at subscription time; the consumer joined with an empty partition assignment and never re-evaluated the regex until its metadata went stale. Confirmed by manually restarting the sink task after topics existed — it immediately picked them up. Fixed generally (not order-dependently) via `consumer.override.metadata.max.age.ms=5000`, so the subscription re-evaluates every 5s regardless of registration order relative to the source connector or `make seed`.
- **`insert.mode=insert`, not `upsert`, with `primary.key.mode=none`.** Matches Raw's designed semantics (Technical-Design.md §3): a faithful, append-only landing of every message, redelivery included. Deduplication and "one current row" semantics are explicitly Silver's concern (M3.2), not Raw's.
- **`scripts/register_connector.sh` generalized to loop over `cdc/*.json`** rather than hardcoding the one source-connector file from M2.3 — the natural DRY extension once a second connector config existed, avoiding a second near-duplicate script.
- **`streaming/` removed from Technical-Design.md §9 entirely, not left `planned`.** Owner's call, made while reviewing this milestone: through M2.2 (Kafka broker), M2.3 (Connect worker), and M2.5 (JDBC sink connector), nothing has ever needed a real file there — every piece of config was either a `docker-compose.yml` env var or a `cdc/*.json` connector config. Rather than continue carrying a speculative placeholder row, it was dropped. `tests/check_structure.sh`'s `DOCUMENTED` list updated to match (removed `streaming`), since the test asserts every documented-as-planned folder actually appears in §9.

## D012 — Transaction metadata: two correlation columns, not one; a real numbered-migration example (2026-07-15)

**Change:** `change/m2.6-transaction-metadata`

**Motivation.** A direct owner question — "how would I connect two Raw records to a single committed ODS transaction?" — surfaced that nothing in the pipeline through M2.5 carries a usable cross-table correlation key. M2.6 closes that gap and, per the owner's explicit framing, is also this project's first real demonstration of schema evolution against an already-provisioned Raw schema.

**Design delta:** `design/Milestones.md` M2.6 rewritten twice during build as empirical testing corrected two wrong assumptions (see Validation). `design/diagrams/` unaffected (no new diagrams this milestone).

**Artifacts:** `cdc/debezium-postgres-source.json` — `provide.transaction.metadata: true`. `cdc/debezium-jdbc-sink.json` — SMT chain extended to add `source.txId` and `transaction.id`, renamed to `_cdc_source_txn_id` and `_cdc_txn_id`. `warehouse/ddl/002_transaction_metadata.sql` — new numbered migration adding both columns to all 4 Raw tables. `tests/integration/test_transaction_metadata.sh`.

**Reversal:** if `total_order`/`data_collection_order` or the `ods.transaction` topic's BEGIN/END events are ever needed (e.g. to confirm every event of a transaction has been observed), extend the same SMT chain and DDL file rather than starting a new migration — this one already establishes the pattern.

**Validation:** choices and rejected alternatives —
- **Two correlation columns, not one — found necessary empirically, not decided up front.** The milestone was drafted expecting `transaction.id` alone (`"<txId>:<lsn>"`) to be the correlation key. Registering the connector and inspecting real landed rows showed this is **wrong**: the LSN suffix advances with every WAL record, so `_cdc_txn_id` differs across rows in the very same transaction (e.g. one simulated file's `raw_files`/`raw_parties`/`raw_file_actions` rows had `782:26782224`, `782:26783752`, `782:26785008` — same leading `782`, different everything after the colon). The actually-stable, directly `=`-comparable value is `source.txId` (a plain integer, present on every Debezium message regardless of `provide.transaction.metadata` — it's core envelope metadata, not part of this feature). Landed on capturing both: `_cdc_txn_id` demonstrates the transaction-metadata feature itself (ties to the `ods.transaction` topic), `_cdc_source_txn_id` is what a real query would actually `WHERE` on.
- **The milestone's original acceptance criterion (`raw_files` and `raw_parties` from `seed.sql` sharing one transaction id) was also wrong, and fixed before writing any test.** `seed.sql` has no explicit `BEGIN`/`COMMIT`; `psql`'s autocommit means its `files`, `parties`, and `file_actions` INSERTs are 3 separate transactions, not 1 — confirmed by reading `ods/seed/seed.sql` and cross-checking against the `ods.transaction` topic's BEGIN/END log (transaction ids `782`, `783`, `784`, one per statement). Owner's choice, offered as the recommended option: prove cross-table correlation with `make simulate COUNT=1` instead, since `simulator/simulate.py`'s `insert_file()` performs exactly one `conn.commit()` after inserting all three tables' rows for a file (psycopg2 defaults to `autocommit=False`) — a genuine single-transaction, cross-table commit, requiring no change to any existing script. Rejected wrapping `seed.sql` in an explicit transaction to fit the original demo, since that would reach back into M1.2 to reshape data around this milestone's needs rather than the reverse.
- **A new, numbered DDL file (`warehouse/ddl/002_transaction_metadata.sql`), not an edit to `001_raw_schema.sql`.** The owner explicitly framed this milestone as "our schema evolution" exercise: `docker-entrypoint-initdb.d` only executes each file once, in order, on a fresh (empty) volume — it does not re-run `001_raw_schema.sql` against an already-initialized one. A real schema change in this project's DDL-mounting model is therefore necessarily a new, additive file, exactly mirroring the `ods/ddl` `001_schema.sql`/`002_replication.sql` precedent from M2.1. Explicit, acknowledged limitation: this only applies automatically to a fresh volume; an existing deployment would need a manual `ALTER TABLE` or a full `make down`/`make up` cycle — not solved here, since this working example has no notion of applying migrations to a running, already-provisioned instance.
- **`total_order`/`data_collection_order` not captured.** They answer "how many events, and where in sequence, within this transaction" — a different question (confirming completeness of a transaction's capture) than "which rows share a transaction," which is all this milestone set out to answer. Adding them now would be speculative ahead of an actual need.

## D013 — Split `persons` (PII) from `users` (operational identity); `parties`/`file_actions` reference real, FK-enforced rows (2026-07-21)

**Change:** `change/amend-user-party-tracking`

**Motivation.** The original `parties` table held a raw `user_id` — an unvalidated integer with no FK and no backing table — conflating a file's role-assignment with operational identity, and providing no place to segregate PII (name, email, SSN) from operational data (team, vendor status). This split introduces `persons` (PII) and `users` (operational identity, FK to `persons`) as distinct tables, with `parties.person_id` and `file_actions.sent_user_id`/`received_user_id` as real, FK-enforced references instead of untyped integers — closing that gap end-to-end from the ODS through CDC, raw landing, and staging.

**Design delta:** `design/Milestones.md` — M1.1 and M1.2's Test/Acceptance/Automated Test Plan/Manual Test Plan sections amended directly in place (not a separate numbered sub-milestone). `design/Technical-Design.md` §3 — `persons`/`users`/`parties` data model updated to match. `design/Requirements.md` A5 — reworded to name the Autoclose System service principal explicitly, now that it's a real `persons`/`users` row rather than an implied `NULL`.

**Artifacts:** `ods/ddl/001_schema.sql` — new `persons` (`person_id` PK, `display_name`, `email` UNIQUE, `ssn_last4`) and `users` (`user_id` PK, `person_id` FK, `team_name`, `is_external_vendor_flag`) tables; `parties.person_id` FK replacing the old unvalidated `parties.user_id`; `file_actions.sent_user_id`/`received_user_id` now FK `users.user_id` (previously plain `bigint`); `audit_events.user_id` now FK `users.user_id`. `ods/ddl/002_replication.sql` — publication extended to `persons`/`users`. `ods/seed/seed.sql` — rewritten to insert a `persons`+`users` row per role (including an "Autoclose System" principal for A5's terminal-step receiver) before referencing them, idempotently (`WHERE NOT EXISTS`). `simulator/workflow.py`/`simulate.py` — `build_file` takes resolved `role_ids` rather than fabricating ids; `simulate.py` maintains a reusable pool of `persons`/`users` per non-`BORROWER` role and a fresh, unique person/user per file for `BORROWER`. `warehouse/ddl/001_raw_schema.sql` — new `raw_persons`/`raw_users`; `raw_parties.person_id` replacing `raw_parties.user_id`. `cdc/debezium-postgres-source.json` — `table.include.list` extended to `public.persons,public.users`. `warehouse/dbt/models/staging/stg_persons.sql`/`.yml`, `stg_users.sql`/`.yml` — new; `stg_parties.sql` updated to select `person_id`. `tests/integration/test_schema_init.sh`, `test_seed_data.sh`, `test_simulator.sh`, `test_jdbc_sink.sh` — joins/assertions updated for the new shape.

**Reversal:** not independently reversible the way a single-file change is — `parties`/`file_actions`/`audit_events`' FKs, the seed script, the simulator, and the staging models all depend on `persons`/`users` existing, so reverting means rolling back all of them together, not `ods/ddl/001_schema.sql` alone.

**Validation:** choices and rejected alternatives —
- **`BORROWER` always gets a fresh, unique `persons`/`users` row per file; every other role draws from a small reusable pool.** A borrower is the file's customer — pooling them would mean the same person appears on multiple files, which isn't a real scenario this simulator should produce. The other 5 roles are internal/vendor staff who plausibly handle many files, so reuse is the more realistic shape.
- **Found and fixed while writing this decision — `test_schema_init.sh` never checked `persons`.** Its table-existence and column-comment loops (`for t in files file_actions parties audit_events users`) omitted `persons` entirely, so despite M1.1's amended acceptance criteria claiming "all 6 tables" verified, only 5 were ever actually checked. Fixed by adding `persons` to both loops.

## D014 — dbt project scaffold: official image pinned to linux/amd64, sources only (2026-07-15)

**Change:** `change/m3.1-dbt-scaffold`

**Motivation.** M3.1 needs a working dbt project wired up against the warehouse before any Silver/Gold/Mart model can be written — proving the connection and the Raw-table source declarations resolve to live data, with zero transform logic yet.

**Design delta:** none beyond what M3.1's draft already specified in `design/Milestones.md`.

**Artifacts:** `warehouse/dbt/dbt_project.yml`, `warehouse/dbt/profiles.yml`, `warehouse/dbt/models/sources.yml` (the 4 Raw tables, `identifier:` mapping `raw_files` etc. to plain source names `files`/`file_actions`/`parties`/`audit_events`). `docker-compose.yml` — new `dbt` service. `Makefile` — `dbt-debug`, `dbt-run` targets. `tests/integration/test_dbt_scaffold.sh`. `scripts/register_connector.sh` — now polls each connector's `/status` after registering and reports `connector.state`/`tasks[0].state`, rather than exiting right after the POST/PUT succeeds.

**Reversal:** if the official image's lag behind current dbt-core (1.8.3 vs. 1.11.12 latest at time of writing) becomes a real limitation for a Silver/Gold feature, swap the image tag; nothing else in this milestone depends on the specific version.

**Validation:** choices and rejected alternatives —
- **`ghcr.io/dbt-labs/dbt-postgres:1.8.latest`, pinned to `platform: linux/amd64`.** Verified empirically before drafting further: every tag checked (`1.8.latest`, `1.9.latest`) has no `arm64` manifest entry, only `amd64` — confirmed via `docker manifest inspect`, not assumed. This is the same Apple Silicon emulation situation Debezium's own docs flagged for `quay.io/debezium/connect` (M2.3), now hit directly. Rejected building a custom multi-arch image from `pip install dbt-core dbt-postgres` — the official image is maintained and already has the adapter pre-installed; the platform pin is a one-line cost, not worth avoiding by taking on image maintenance.
- **`profiles.yml` mounted alongside `dbt_project.yml` in the same directory, found via `DBT_PROFILES_DIR=/usr/app/dbt`**, rather than dbt's default `~/.dbt/profiles.yml` location. Confirmed via `docker inspect` that the image sets no `DBT_PROFILES_DIR` and no default profiles file exists in the image — the project's own volume mount is the natural place to put it, keeping one directory as the complete, version-controlled unit (matching `.env`'s committed-local-defaults pattern; not for production use).
- **`identifier:` mapping in `sources.yml`** (`source('raw', 'files')` resolves to the real table `raw_files`), rather than naming the dbt source the same as the physical table. Keeps model-facing source names aligned with the layer name ("Raw") used throughout Technical-Design.md §3, independent of the `raw_` prefix that exists for a different reason (M2.5's `RegexRouter` topic-to-table naming).
- **Acceptance proven against live data, not just `dbt debug`.** An early risk: `dbt debug`/`dbt run` passing only proves the *connection* and *config* are valid — it says nothing about whether the source declaration in `sources.yml` actually matches real column/table names in `warehouse/ddl/`. `tests/integration/test_dbt_scaffold.sh` additionally runs `dbt show --inline "select count(*) from {{ source('raw','files') }}"` against a freshly seeded row and asserts the count, so a typo'd `identifier:` would fail the test even though `dbt debug` would still pass.
- **`dbt show` output parsed via plain-text `--no-use-colors` + a `grep -E '\| [0-9]+ \|$'` pattern, not `--output json`.** Tried `--log-format json` first; while it does emit structured JSON lines, the actual result table is embedded as an escaped string inside one log event's `data.preview`/`info.msg` field, no simpler to parse reliably in `sh` than matching the plain-text table's numeric row directly once ANSI color codes are disabled.
- **No `raw` schema rename.** Raw tables stay in `public`, where M2.5 put them; `sources.yml`'s `schema: public` just points at where they already are. Renaming schemas is a bigger, unrelated change with no present need.
- **`register_connector.sh` now polls and reports `RUNNING` status, found missing during owner review of M3.1's manual test plan.** The manual steps went straight from `make up` to `make simulate COUNT=1`, skipping `make register-connector` entirely — without it, nothing bridges the ODS to the warehouse and the following `dbt show` step would return 0, not 1. Fixing the docs alone wasn't enough: `register_connector.sh` only ever confirmed the POST/PUT succeeded, not that the connector actually reached `RUNNING` (the same connector/task race documented in M2.3's acceptance criteria). Extended the script to poll `/connectors/<name>/status` for up to 30s and print both `connector.state` and `tasks[0].state`, so `make register-connector` itself tells you whether to expect data after a subsequent seed or simulate — rejected adding a separate composite `make bootstrap` command, since fixing the one existing command to report accurately covers the actual gap without introducing a second way to do the same thing.

## D015 — Adopt dbt-native project structure as the standard; amend M3.1 to match (2026-07-15)

**Change:** `change/m3.1-dbt-native-amend`

**Motivation.** Reviewing M3.1 against dbt's own "How we structure our dbt projects" guide surfaced that the project's layer vocabulary (Raw → Silver → Gold → Mart) is a bespoke four-layer scheme that does not map cleanly onto either dbt's native layers (`sources` → `staging` → `intermediate` → `marts`) or the three-layer medallion pattern (Bronze/Silver/Gold), and that this ambiguity would drive folder layout, file naming, and — critically — *where deduplication logic belongs* before any real model is written. Rather than carry a non-standard structure forward into M3.2+, dbt's native conventions are adopted as the project standard now, while the only conflict already committed (M3.1's) is small enough to fix as a scoped amendment.

**Design delta:** dbt-native structure becomes the standard, with medallion terms retained only as a cross-reference. Mapping:

| dbt (standard) | Path / prefix | Materialization | Medallion (cross-ref) |
| --- | --- | --- | --- |
| sources | `raw` source | n/a (landed) | Bronze |
| staging | `models/staging/stg_*` | view | Silver |
| intermediate | `models/intermediate/int_*` | view (or ephemeral) | Silver |
| marts | `models/marts/fct_*` / `dim_*` | table | Gold |

The medallion mapping is deliberately many-to-one (Bronze = sources, **Silver = staging + intermediate**, Gold = marts) rather than pretending the old four labels map 1:1 to medallion's three. dbt terms are the standard going forward; `Requirements.md` is untouched.

**Artifacts (this change):** `warehouse/dbt/models/sources.yml` → `warehouse/dbt/models/staging/__sources.yml` (relocation via `git mv`, content unchanged apart from its header comment). `design/Milestones.md` M3.1 — an "Amended implementation" note recording the relocation and the standard adopted. `design/Technical-Design.md` — §3 layer sections renamed to the dbt-native standard (Silver/Gold/Metric Mart → Staging/Intermediate/Marts), a new "Warehouse layers (dbt-native; medallion cross-reference)" map added, the served objects renamed to `fct_step_turnaround` + `agg_step_turnaround`, the §1/§2/§4/§6/§9 flow references converted, and the §7 materialization open-question marked resolved by this decision (staging/intermediate = views, marts = tables). The Technical-Design update was folded into this change at the owner's request; it is documentation of the standard, distinct from the model implementation below. `design/Milestones.md` was also converted throughout to the dbt-native vocabulary (M3.2 Staging / M3.3 Intermediate / M3.4 Marts, served objects `fct_step_turnaround` / `agg_step_turnaround`, and the M2.4/M2.5/M3.1/M4/M5/M6 forward-references to "the staging layer" and "marts"). `Makefile`, `warehouse/ddl/001_raw_schema.sql`, and `docker-compose.yml` — code comments converted from "Raw → Silver → Gold → Mart" to the dbt-native layer names.

**Explicitly deferred to later changes (not done here):**
- The `stg_`/`int_`/`fct_`/`dim_` model *implementations* — the dbt SQL models themselves for M3.2 (staging), M3.3 (intermediate), M3.4 (marts). Those models do not exist yet; building them, and expanding the M3.2–M3.4 milestone stubs from their current one-line form into full draft specs, is the *next* change's scope, not this one's. The milestone vocabulary and served-object names (`fct_step_turnaround`, `agg_step_turnaround`) are already converted in this change — only the model code and the detailed spec drafts remain.
- Per-folder materialization defaults (`+materialized`) in `dbt_project.yml` — governs models that do not exist yet; lands with M3.2.
- Per-layer warehouse schemas (`staging`/`marts` schemas vs. everything in `public`) — remains deferred per D014; not reopened by this decision.

**Reversal:** the relocation is a pure `git mv`; moving `__sources.yml` back to `models/` would restore the prior state (dbt resolves sources by declaration, not file path, so location is a convention choice, not a functional dependency — confirmed: `dbt list --resource-type source` returns all four `raw.*` sources from the new location, and `test_dbt_scaffold.sh` still passes).

**Validation:** choices and rejected alternatives —
- **Scoped forward amendment, not a rollback of M3.1.** M3.1 is already pushed to `origin/main`; its conflict surface is just two files, and it built zero models (the staging/intermediate/marts folders and their models only appear in M3.2+). Reverting or force-pushing to "rebuild M3.1 dbt-native" would either rewrite public history or undo M3.1's convention-agnostic work (the `register_connector.sh` status fix, the `dbt` service) only to re-add it — disproportionate to relocating one file. Rejected both in favor of amending M3.1's artifacts in place and recording the amendment.
- **`models/staging/__sources.yml`, not `models/sources.yml`.** dbt's guide colocates source definitions in the staging folder with a `__sources.yml` (double-underscore) name; adopting the standard means adopting its file placement, even though dbt would resolve the sources from anywhere under `model-paths`.
- **Per-folder materialization config deliberately not added here.** It configures how *models* materialize; with zero models it would be config for nothing, and it belongs to M3.2's scope by the owner's explicit instruction to keep this amendment to M3.1's own conflicts.

## D016 — Simulator status-lifecycle updates: two-phase insert-then-update, WIP not OPEN (2026-07-15)

**Change:** `change/m2.8-lifecycle-updates`

**Motivation.** Every test through M2.6 was insert-only, so Debezium's and the JDBC sink's `UPDATE` (`op=u`) path was completely unverified, and M3.2's staging dedup had nothing but synthetic rows to prove against — the pipeline never produced a second version of any key. M2.8 makes the simulator emit a realistic status lifecycle so multiple-versions-per-key arise naturally in Raw.

**Design delta:** `design/Milestones.md` — M2.8 added (CDC theme, after the deferred M2.7); M3.2's draft revised to test dedup against real multi-version data and to correct its "out-of-order" claim to "redelivery + multiple versions." No Technical-Design change: the no-hard-deletes assumption (§2) still holds; updates were always allowed by the schema.

**Artifacts:** `simulator/workflow.py` — new pure `open_state(file_data)` deriving the WIP snapshot. `simulator/simulate.py` — `insert_file()` rewritten as two transactions (insert WIP snapshot, then update to closed). `simulator/tests/test_workflow.py` — `OpenStateTests`. `tests/integration/test_lifecycle_updates.sh` — new. `tests/integration/test_transaction_metadata.sh` — revised for the lifecycle.

**Reversal:** the lifecycle lives entirely in `insert_file()`; reverting to a single-transaction all-final insert restores insert-only behavior. `open_state()` is pure and side-effect-free, so it can be dropped independently.

**Validation:** choices and rejected alternatives —
- **`WIP`, not `OPEN`, for the open state — caught before coding by reading the schema, not assumed.** The approved M2.8 draft said `OPEN`, but `ods/ddl/001_schema.sql`'s `files.status` CHECK allows only `('WIP', 'CLOSED')`. Inserting `OPEN` would have failed the constraint at runtime; the draft and all M2.8/M3.2 references were corrected to `WIP` before implementation.
- **Two transactions (insert commit, then update commit), not one.** The insert phase keeps its single commit across `files`/`parties`/`file_actions` so M2.6's cross-table correlation still holds for the create rows (all share one `source.txId`); the update phase is a separate commit with its own txId. Verified in `test_lifecycle_updates.sh`: create rows across all three tables share one `_cdc_source_txn_id`, update rows share a different one.
- **`make simulate` gets the lifecycle; `make seed` stays insert-only.** Owner's call (default lifecycle, most faithful). Keeping `seed` a fixed single-version fixture means M1.2/M2.5's seed-based integration tests (`test_seed_data.sh`, `test_jdbc_sink.sh`, `test_debezium_connector.sh`) needed no change — confirmed by full regression. Only `test_transaction_metadata.sh` (which uses `simulate`) required revision.
- **`action_type` stays `COMPLETE` through the lifecycle; the WIP→CLOSED transition is carried by `files.status` + `file_actions.received_at`, not by a `START`→`COMPLETE` flip.** Kept the change minimal and matched the approved scope; the `START` enum value stays unused rather than speculatively wiring a second state dimension that no downstream metric consumes yet.
- **`parties` stay insert-only.** A party-role assignment doesn't mutate over a file's lifecycle in this workflow, so there was no faithful update to emit — `open_state()` passes `parties` through unchanged.
- **ODS `UPDATE`s modify rows in place, so the ODS final state is identical to the old insert-only simulator** (1 file / 4 actions / 6 parties, closed). The extra versions exist only in the append-only Raw landing via CDC — confirmed by asserting the ODS counts/final state in `test_lifecycle_updates.sh` alongside the 2-per-key Raw version counts. This is why ODS-querying tests were unaffected.
- **Default `REPLICA IDENTITY` suffices.** The sink lands only the `after` image (via `ExtractNewRecordState`), which Debezium always provides for updates; `REPLICA IDENTITY FULL` (needed for a full `before` image) stays the deferred M2.7's concern and was not touched.
- **Create/update `_cdc_ts` compared with `>=`, not `>`.** Debezium's event timestamp is millisecond-resolution, so a fast insert-then-update can stamp both events in the same millisecond; the test asserts the update's `_cdc_ts` is `>=` the create's rather than strictly greater, avoiding a false failure on a sub-millisecond lifecycle.

## D017 — Staging models: shared dedup macro, real lifecycle data as the primary proof (2026-07-15)

**Change:** `change/m3.2-staging-models`

**Motivation.** M3.2 needs the first real dbt models: one current row per business entity in `stg_files`/`stg_file_actions`/`stg_parties`, collapsing the append-only Raw landing. With M2.8 merged, this is also the first milestone where dedup can be proven against real multi-version data instead of only synthetic rows.

**Design delta:** none beyond what M3.2's revised draft already specified in `design/Milestones.md`. `tests/integration/test_dbt_scaffold.sh` (M3.1) required a small update — see Validation.

**Artifacts:** `warehouse/dbt/macros/dedup_latest.sql` — shared `row_number() over (partition by <key> order by _cdc_ts desc) = 1` macro. `warehouse/dbt/models/staging/stg_files.sql`, `stg_file_actions.sql`, `stg_parties.sql` — each a thin wrapper selecting only business columns from the deduped Raw source. `stg_files.yml`/`stg_file_actions.yml`/`stg_parties.yml` — `unique`+`not_null` tests on each entity key, the project's first `dbt test`s. `warehouse/dbt/dbt_project.yml` — per-folder `+materialized` config for `staging`/`intermediate`/`marts` (the M3.1-deferred config, added now that `staging/` has models). `tests/integration/test_stg_dedup.sh` — new. `tests/integration/test_dbt_scaffold.sh` — revised assertion (see Validation).

**Reversal:** each staging model is independent (one source, one dedup call, one column projection); dropping or reworking one doesn't affect the others. The macro is reused, not inherited — a model can bypass it if a future entity's dedup logic diverges.

**Validation:** choices and rejected alternatives —
- **Dedup proven primarily against real M2.8 lifecycle data, with only the redelivery case left synthetic.** `test_stg_dedup.sh` runs `make simulate COUNT=1` and asserts the real `WIP`→`CLOSED` version pair collapses correctly — this is the payoff of building M2.8 before M3.2, per the owner's explicit sequencing choice. Redelivery (two byte-for-byte identical messages) still has no deterministic real trigger in this pipeline, so one synthetic duplicate row (reserved `file_id=999999`, which the simulator never generates) covers that case alone, down from M2.8-era drafts that synthesized the entire dedup proof.
- **One shared macro (`dedup_latest`), not three copies of the same window function.** The three models are otherwise nearly identical (source → dedup → column projection); factoring the dedup logic once means a future correctness fix (e.g. a tiebreak column) is made in one place. Verified via `docker compose exec ... \dv` that all three still materialize as views under the shared macro, not just by reading the SQL.
- **`dbt_project.yml` per-folder config added for `staging`, `intermediate`, and `marts` together, even though only `staging/` has models yet.** dbt's config blocks are declarative and inert for paths with no models — confirmed empirically (`dbt run` succeeded cleanly with only `staging/` populated, no errors referencing the not-yet-existing `intermediate`/`marts` folders). Configuring all three now avoids revisiting this file again for the same reason in M3.3/M3.4.
- **Found and fixed while building — `test_dbt_scaffold.sh`'s "0 models" assertion broke as an expected, not accidental, consequence of M3.2.** Its `grep -q "Nothing to do"` check was specifically pinned to the zero-model state M3.1 built against; once `stg_*` models exist, `dbt run` legitimately produces `"Completed successfully"` with `PASS=3` instead. Fixed by asserting the latter (confirmed against real `dbt run` output before pinning the string, not assumed) — this proves the same thing the original check intended (a clean, non-erroring run) without freezing a model count that grows every M3.x milestone. `design/Milestones.md` M3.1 records this as a superseded-but-historically-accurate acceptance criterion rather than editing the original claim.
- **Test-first discipline verified explicitly despite writing models before the test file:** models were held out of `warehouse/dbt/models/staging/` and `test_stg_dedup.sh` was re-run to confirm it fails for the right reason (missing relations `stg_files`/`stg_file_actions`/`stg_parties`, not a script bug) before restoring them and confirming a full pass.
